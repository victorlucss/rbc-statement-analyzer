...
from app.visa import is_visa, parse_visa, transactionsToJson
...
def main():
    ...
    transactions = sorted(
        [
            tx
            for file in files
            for tx in parse_pdf(file, config.get("categories"), ["PAYMENT"])
        ],
    ...
    write_file(transactionsToJson(out_str), 'out.json')
